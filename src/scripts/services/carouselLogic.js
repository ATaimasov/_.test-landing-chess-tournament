function carousel(section, options = {
    ...
    function generateSlides() {
        ...
        for (let i = 0; i < options.slideConfiguration.length; i++) {
            ...
            for (let j = 0; j < options.slideConfiguration[i] && itemIndex < items.length; j++) {
    ...
    function updateButtonStates() {
        if (options.infinity) {
            ...
            nextButton.classList.toggle('carousel__control--disabled', slideIndex === totalSlides - 1);
        }
    }
...
export function clearCarousel(sectionId) {
    ...
    if (carouselElement) {
        ...
        const items = Array.from(carouselElement.querySelectorAll('.carousel__item'));
